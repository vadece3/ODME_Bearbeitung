                        ...
                        if (!ODMEEditor.nodeAddDetector.equals("delete")) {
                            ...
                            if (varCell.isVertex()) {
                                ...
                                if (!(treePathForVariable == null)) {
                                    ...
                                    /*
                                     * Author: Lionce Vadece
                                     * This is to enable the Distribution table to react
                                     * when a node is clicked on
                                     */
                                    String[] nodesToSelectedNodeDistribution = new String[100];
                                    int bbb = 0;
                                    ...
                                    for (TreePath key : DynamicTree.distributionMap.keySet()) {
                                        ...
                                        for (String value : DynamicTree.distributionMap.get(key)) {
                                            ...
                                            TreeNode[] nodes3 = currentNode2.getPath();
                                            ...
                                            if (a == 1) {
                                                nodesToSelectedNodeDistribution[bbb] = value;
                                                bbb++;
                                            }
                                        }
                                    }
                                    ...
                                    nodesToSelectedNodeBehaviour = Arrays.stream(nodesToSelectedNodeBehaviour)
                                            .filter(s -> (s != null && s.length() > 0))
                                            .toArray(String[]::new);
                                    nodesToSelectedNodeDistribution = Arrays.stream(nodesToSelectedNodeDistribution)
                                            .filter(s -> (s != null && s.length() > 0))
                                            .toArray(String[]::new);

                                    Arrays.parallelSort(nodesToSelectedNode);
                                    Arrays.parallelSort(nodesToSelectedNodeBehaviour);
                                    Arrays.parallelSort(nodesToSelectedNodeDistribution);
                                    ...
                                    ODMEEditor.scenarioDistribution
                                            .showNodeValuesInDistributionTable(currentNode.toString(),
                                                    nodesToSelectedNodeDistribution);
                                    ...
                                    String cellName = (String) varCell.getValue();
                                    if (cellName.endsWith("Dec")) {
                                        ODMEEditor.treePanel.showInterConstraintsInTable(treePathForVariable);
                                        ODMEEditor.scenarioIntraEntityConstraint.setNullToAllRows();
                                    }
                                    else {
                                        ODMEEditor.treePanel.showIntraConstraintsInTable(treePathForVariable);

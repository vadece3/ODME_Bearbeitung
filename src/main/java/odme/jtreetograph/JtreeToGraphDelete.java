...
public class JtreeToGraphDelete {
	...
	public static void deleteConstraintFromScenarioTableForUpdate(mxCell cellForAddingVariable, String variableName,
		...
		if ((variableName != null) && (!variableName.trim().isEmpty())) {
			...
			if (yv == 1) {
				DynamicTree.interConstraintsList.remove(keyDel, variableName); // for removing only one values
				yv = 0;
			}

			// have to call a function to refresh the table view
			DynamicTree.interConstraintsList.put(treePathForVariable, variableNameNew);
			// ---------------------------------------------------end
	...
	public static void deleteInterEntityConstraintFromGraphPopup(Object pos) {
		// deleting all the variables of a node
		...
		if (yv == 1) {
			for (TreePath k : delKeys) {
				DynamicTree.interConstraintsList.asMap().remove(k);
				yv = 0;
			}
		}
	...
	public static void deleteAllIntraEntityConstraintFromGraphPopup(Object pos) {
		mxCell cellForAddingVariable = (mxCell) pos;
		pathToRoot.add((String) cellForAddingVariable.getValue());
		JtreeToGraphConvert.nodeToRootPathVar(cellForAddingVariable);
		...
		if (yv == 1) {
			for (TreePath k : delKeys) {
				...
				DynamicTree.intraConstraintsList.asMap().remove(k);
				yv = 0;
			}
		}

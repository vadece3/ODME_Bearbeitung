...
public class JtreeToGraphConvert {
    ...
    public static void nodeToRootPathDist(mxCell cell) {
        Object[] incoming = graph.getIncomingEdges(cell);
        if (incoming.length != 0) {
            Object source = graph.getModel().getTerminal(incoming[incoming.length - 1], true);
            mxCell sourceCell = (mxCell) source;
            pathToRoot.add((String) sourceCell.getValue());
            nodeToRootPathDist(sourceCell);
        }
    }
    ...
    public static void rootToEndNodeDistribution() {
        Object[] cells = graph.getChildVertices(graph.getDefaultParent());
        mxCell rootcell = null;
        ...
        if (rootcell != null) {
            JtreeToGraphNext.nextChildNodeForDistribution(rootcell);
        }
    }

    public static void rootToEndNodeInterConstraint() {
        Object[] cells = graph.getChildVertices(graph.getDefaultParent());
        mxCell rootcell = null;
        ...
        if (rootcell != null) {
            JtreeToGraphNext.nextChildNodeForInterConstraint(rootcell);
        }
    }

    public static void rootToEndNodeIntraConstraint() {
        Object[] cells = graph.getChildVertices(graph.getDefaultParent());
        mxCell rootcell = null;
        ...
        if (rootcell != null) {
            JtreeToGraphNext.nextChildNodeForIntraConstraint(rootcell);
        }
    }
    ...
    public static void rootToEndDistributionAddition(mxCell varCell) { // Author: Vadece Kamdem
        if (varCell.isVertex()) {
            pathToRoot.add((String) varCell.getValue());
            nodeToRootPathVar(varCell);
            ...
            nodesToSelectedNode =
                    Arrays.stream(nodesToSelectedNode).filter(s -> (s != null && s.length() > 0))
                            .toArray(String[]::new);
            for (String value : nodesToSelectedNode) {
                variables = variables + "<" + value + "Distion/>";
            }

            // treePathForDistribution
            FileConvertion fileConversion = new FileConvertion();
            fileConversion.distributionAdditionToNode(treePathForVariable, variables);

            pathToRoot.clear();
        }
    }

    public static void rootToEndIntraConstraintAddition(mxCell varCell) { // Author: Vadece Kamdem
        if (varCell.isVertex()) {
            pathToRoot.add((String) varCell.getValue());
            nodeToRootPathVar(varCell);
            ...
            for (TreePath key : DynamicTree.intraConstraintsList.keySet()) {
                int a = 0;

                for (String value : DynamicTree.intraConstraintsList.get(key)) {
            ...
            nodesToSelectedNode =
                    Arrays.stream(nodesToSelectedNode).filter(s -> (s != null && s.length() > 0))
                            .toArray(String[]::new);
            for (String value : nodesToSelectedNode) {
                variables = variables + "<" + value + "IntraCon/>" + "\n";
            }

            // treePathForDistribution
            FileConvertion fileConversion = new FileConvertion();
            fileConversion.intraConstraintAdditionToNode(treePathForVariable, variables);

            pathToRoot.clear();
        }
    }

    public static void rootToEndInterConstraintAddition(mxCell varCell) { // Author: Vadece Kamdem
        if (varCell.isVertex()) {
            pathToRoot.add((String) varCell.getValue());
            nodeToRootPathVar(varCell);
            ...
            // treePathForDistribution
            FileConvertion fileConversion = new FileConvertion();
            fileConversion.interConstraintAdditionToNode(treePathForVariable, variables);
...
//
//            // Get the path for the currently processed cell
//            pathToRoot.add((String) varCell.getValue());
//            nodeToRootPathVar(varCell);
//            String[] stringArray = pathToRoot.toArray(new String[0]);
...
//            }
//
//            DefaultMutableTreeNode currentNode = (DefaultMutableTreeNode) (treePathForVariable.getLastPathComponent());
//            TreeNode[] nodes = currentNode.getPath();
//
//            // --- PART 1: Process Inter-Entity Constraints (using your original, robust logic) ---
//            for (TreePath key : DynamicTree.interConstraintsList.keySet()) {
//                DefaultMutableTreeNode keyNode = (DefaultMutableTreeNode) (key.getLastPathComponent());
//                TreeNode[] keyNodes = keyNode.getPath();
//                boolean pathIsEqual = false;
//
//                if (nodes.length == keyNodes.length) {
//                    pathIsEqual = true; // Assume true until a mismatch is found
//                    for (int i = 0; i < nodes.length; i++) {
//                        if (!nodes[i].toString().equals(keyNodes[i].toString())) {
//                            pathIsEqual = false;
//                            break;
//                        }
//                    }
...
//            }
//
//            // --- PART 2: Process Intra-Entity Constraints (using the same robust logic) ---
//            for (TreePath key : DynamicTree.intraConstraintsList.keySet()) {
//                DefaultMutableTreeNode keyNode = (DefaultMutableTreeNode) (key.getLastPathComponent());
//                TreeNode[] keyNodes = keyNode.getPath();
//                boolean pathIsEqual = false;
...
//            }
//
//            // If any constraints were found, add them to the file
//            if (variables.length() > 0) {
//                fileConversion.constraintAdditionToNode(selectedNode, variables.toString());
//            }
//
//            pathToRoot.clear();
//        }
//    }
    ...
    // This is the new private helper method. Add it to the same class.
    private static String getConstraintsAsString(TreePath currentPath, Multimap<TreePath, String> constraintList) {
        StringBuilder variables = new StringBuilder();
        TreeNode[] currentNodes = ((DefaultMutableTreeNode) currentPath.getLastPathComponent()).getPath();

        for (TreePath key : constraintList.keySet()) {
            TreeNode[] keyNodes = ((DefaultMutableTreeNode) key.getLastPathComponent()).getPath();
            boolean pathIsEqual = false;

            if (currentNodes.length == keyNodes.length) {
                pathIsEqual = true; // Assume true until a mismatch is found
                for (int i = 0; i < currentNodes.length; i++) {
                    if (!currentNodes[i].toString().equals(keyNodes[i].toString())) {
                        pathIsEqual = false;
                        break;
                    }
                }
            ...
            if (pathIsEqual) {
                for (String value : constraintList.get(key)) {
                    variables.append("<").append(value).append("Con/>").append("\n");
                }
            }
        }
